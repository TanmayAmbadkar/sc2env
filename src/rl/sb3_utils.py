...
def learn(
    ...
    while policy.num_timesteps < total_timesteps:
        ...
        test_policy(policy, policy.env, n_eval_episodes=n_eval_episodes)

    callback.on_training_end()

    return policy

def test_policy(policy, env, n_eval_episodes=3):
    """
    Test the policy on the given environment.

    Parameters:
    policy: The policy to be tested.
    env: The environment to test the policy on.
    n_eval_episodes (int): The number of episodes to test the policy.
    deterministic (bool): Whether to use deterministic or stochastic actions.
    render (bool): Whether to render the environment.

    Returns:
    float: The mean reward over the test episodes.
    """
    # Switch to eval mode
    ...
    mean_episode_reward = safe_mean(all_episode_rewards)
